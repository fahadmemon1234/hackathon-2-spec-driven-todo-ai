...
def test_add_command_integration():
    ...
    # Mock the input to simulate user typing "add Buy milk Grocery shopping"
    with patch('builtins.input', side_effect=['add Buy milk Grocery shopping', 'quit']):
...
def test_add_command_output():
    ...
    assert output.startswith("Task added with ID:")
...
def test_update_command_integration():
    ...
    f = io.StringIO()
    with contextlib.redirect_stdout(f):
        app.process_command(f"update {task_id} Buy bread Bakery items")
...
def test_update_command_validation_error():
    ...
    f = io.StringIO()
    with contextlib.redirect_stdout(f):
        app.process_command(f"update {task_id} " + "A"*101 + " New description")
